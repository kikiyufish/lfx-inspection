...
function InspectionPageContent() {
    ...
    const [expandedCategory, setExpandedCategory] = useState<string | null>(inspectionCategories[0]?.id || null);
    ...
    const saveDraft = useCallback(() => {
        ...
        saveTimerRef.current = setTimeout(() => {
            const draft: DraftData = {
                ...
                itemData: Object.fromEntries(Object.entries(itemData).map(([k, v]) => [k, {
                    ...v
                }])),
    ...
    const restoreDraft = useCallback(() => {
        ...
        if (draftInfo.currentStep)
            setCurrentStep(draftInfo.currentStep);
        ...
        const fetchTodayRecords = async () => {
            ...
            try {
                ...
                const res = await fetch(
                    `/api/inspections?supervisor=${encodeURIComponent(supervisorName)}&date=${today}`
                );
    ...
    useEffect(() => {
        if (editId) {
            ...
            fetch(`/api/inspections/${editId}`).then(res => res.json()).then(result => {
            ...
            }).catch(err => console.error("Failed to load inspection:", err)).finally(() => setIsLoading(false));
        }
    }, [editId]);
    ...
    const totalScore = allItems.reduce((sum, item) => sum + (itemData[item.itemNumber]?.actual_score || 0), 0);
    ...
    const handleSubmit = async () => {
        ...
        if (unscoredItems.length > 0) {
            ...
            const category = inspectionCategories.find(cat => cat.items.some(i => i.itemNumber === firstUnscored.itemNumber));
    ...
    if (currentStep === "info") {
        return (
            <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
                {}
                <div
                    className="bg-gradient-to-r from-amber-700 to-amber-600 text-white px-4 py-6 shadow-lg">
                    <div className="max-w-lg mx-auto">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                <a
                                    href="/history"
                                    className="px-3 py-1.5 bg-white/15 rounded-lg text-xs font-medium hover:bg-white/25 transition-colors">历史记录
                                                    </a>
                                <a
                                    href="/stats"
                                    className="px-3 py-1.5 bg-white/15 rounded-lg text-xs font-medium hover:bg-white/25 transition-colors">统计报告
                                                    </a>
                                <a
                                    href="/admin"
                ...
                <div className="max-w-lg mx-auto px-4 py-6">
                    {}
                    {draftInfo && <div
                        className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 mb-6 shadow-sm">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {draftInfo.storeName && <p className="text-sm text-amber-700 mb-1">门店：{draftInfo.storeName}</p>}
                                {draftInfo.supervisorName && <p className="text-xs text-amber-500 mb-3">督导：{draftInfo.supervisorName}</p>}
                                <div className="flex gap-2">
                                    <button
                                        onClick={restoreDraft}
                                        className="px-4 py-2 bg-amber-500 text-white text-sm font-medium rounded-lg hover:bg-amber-600 active:scale-[0.98] transition-all">继续填写
                                                            </button>
                                    <button
                                        onClick={discardDraft}
                                        className="px-4 py-2 bg-white text-amber-600 text-sm font-medium rounded-lg border border-amber-200 hover:bg-amber-50 active:scale-[0.98] transition-all">放弃草稿
                                                            </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <h2
                            ...
                            <span
                                className="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center text-amber-700 text-sm font-bold">1</span>门店信息
                                        </h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">督导姓名 <span className="text-red-500">*</span>
                                </label>
                                <select
                                    ...
                                    <option value="浦晨鸣">浦晨鸣</option>
                                    <option value="张瑜">张瑜</option>
                                    <option value="陈怡伶">陈怡伶</option>
                                    <option value="桂雅芳">桂雅芳</option>
                                    <option value="陶海诚">陶海诚</option>
                                    <option value="王玮">王玮</option>
                                </select>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">门店名称 <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    value={storeName}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">检查日期 <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="date"
                            ...
                            {}
                            {supervisorName.trim() && <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                                ...
                                {!loadingToday && todayRecords.length === 0 && <p className="text-xs text-blue-500">今天暂无检查记录，开始新的检查吧</p>}
                                {!loadingToday && todayRecords.length > 0 && <div className="space-y-2">
                                    {todayRecords.map(record => <div
                                        key={record.id}
                                        ...
                                        <div className="flex gap-1.5 ml-2">
                                            <button
                                                ...
                                                className="px-2.5 py-1 text-xs text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors">查看
                                                                            </button>
                                            {record.edit_count === 0 && <button
                                                onClick={() => router.push(`/?edit=${record.id}`)}
                                                className="px-2.5 py-1 text-xs text-amber-600 bg-amber-50 rounded-md hover:bg-amber-100 transition-colors">修改
                                                                              </button>}
                                        </div>
                                    </div>)}
                                </div>}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">区域
                                                    </label>
                                <select
                                    value={region}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1.5">区域经理</label>
                                <select
                    ...
                    <div
                        ...
                        <h2
                            ...
                            <span
                                className="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center text-amber-700 text-sm font-bold">2</span>检查概览
                                        </h2>
                        <div className="grid grid-cols-2 gap-3">
                            {inspectionCategories.map(
                                cat => <div key={cat.id} className="bg-gray-50 rounded-xl p-3 text-center">
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-amber-600 to-amber-500 text-white font-semibold rounded-2xl shadow-lg shadow-amber-200 active:scale-[0.98] transition-transform text-lg">开始巡店检查
                                  </button>
                </div>
            </div>
        );
            ...
            <div className="bg-white border-b border-gray-200 sticky top-0 z-30">
                <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {hasUnsavedChanges && !isEditMode && <span className="text-[10px] text-green-500 flex items-center gap-0.5">
            ...
            <div className="max-w-lg mx-auto px-4 py-4">
                ...
                <div
                    ...
                    <div
                        ...
                        <span
                            className={`text-xs font-medium ${scoredItems.size === allItems.length ? "text-green-600" : "text-orange-500"}`}>
                            {scoredItems.size}/{allItems.length}项已评分
                                          {scoredItems.size < allItems.length && <span className="ml-1">（还有{allItems.length - scoredItems.size}项未评分）</span>}
                        </span>
                    </div>
                </div>
                {}
                {inspectionCategories.map(category => <CategorySection
                    key={category.id}
                    category={category}
                    isExpanded={expandedCategory === category.id}
                    onToggle={() => setExpandedCategory(expandedCategory === category.id ? null : category.id)}
                    itemData={itemData}
                    categoryScore={category.items.reduce((sum, item) => sum + (itemData[item.itemNumber]?.actual_score || 0), 0)}
                    ...
                    renderItem={item => <div
                        ...
                        <div className="flex items-start gap-2 mb-3">
                            <span
                                className="shrink-0 w-6 h-6 bg-amber-100 text-amber-700 rounded-full flex items-center justify-center text-xs font-bold mt-0.5">
                                {item.itemNumber}
                            </span>
                        ...
                        <div className="mt-3">
                            <textarea
                                ...
                                onChange={e => updateItemData(item.itemNumber, "notes", e.target.value)}
                        ...
                        {}
                        {(itemData[item.itemNumber]?.actual_score ?? 0) < item.maxScore && <div className="mt-3 p-3 bg-orange-50 rounded-xl border border-orange-100">
                            <div className="flex items-center gap-2 mb-2">
                                <svg
                            ...
                            <textarea
                                ...
                                onChange={e => updateItemData(item.itemNumber, "rectification", e.target.value)}
                                placeholder="请描述整改要求..."
                        ...
                        <PhotoUpload
                            ...
                            onPhotosChange={keys => updateItemData(item.itemNumber, "photo_keys", keys)} />
                    </div>} />)}
            </div>
            {}
            <div
...
export default function InspectionPage() {
    return (
        <Suspense
            fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-gray-500">加载中...</div></div>}>
