...
export default function StatsPage() {
  ...
  const [exportingSupervisor, setExportingSupervisor] = useState(false);
  ...
  // 导出门店评分汇总记录
  const exportSupervisorReport = async () => {
    if (exportingSupervisor || fullData.length === 0) return;
    setExportingSupervisor(true);
    try {
      const today = new Date();
      const dateStr = today.toISOString().split('T')[0];
      const response = await fetch(`/api/supervisor-report/excel?date=${dateStr}`, {
        credentials: 'include',
      });
      if (!response.ok) {
      ...
      link.href = url;
      link.download = data.filename || `门店评分汇总记录_${dateStr}.xlsx`;
      link.click();
      URL.revokeObjectURL(url);
      alert('导出成功！');
    } catch (error: any) {
      console.error('导出失败:', error);
      alert(error.message || '导出失败，请查看控制台错误信息');
    } finally {
      setExportingSupervisor(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 py-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm mb-4">
          ...
          <div className="flex gap-3 mt-3">
            ...
            <button
              onClick={exportSupervisorReport}
              disabled={exportingSupervisor || fullData.length === 0}
              className="flex-1 bg-amber-600 text-white py-3 rounded-xl font-medium flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {exportingSupervisor ? (
                <>
                  <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
